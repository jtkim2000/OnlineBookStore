...
1. 도서를 대여 신청한다.
2. 신청한 도서가 있으면 대여한다.
3. 특정 책은 다른 책을 대여한 이력 있어야 대여된다
4. 대여 후 대여 수량만큼 도서 재고를 감한다.
5. 도서가 없으면 신청자에게 도서가 없음을 알린다.
6. 대여 못받은 신청자 정보를 별도 저장한다.
7. 사서는 부족한 도서를 구매한다.
8. 관리센터는 대여상태, 도서재고 상태를 수집한다.
9. 신청자가 도서를 반납한다.
10. 반납 후 반납 수량만큼 도서 재고를 증가시킨다.
11. 반납 시 반납 도서의 상태를 점검하여 good/bad 상태를 수정한다.
12. Bad인 경우 반납자 등급을 강등한다.good/bad
13. 사서는 책의 상태가 Bad인 경우 반드시 해당 책 구매한다. 
14. 책이 구매되면 책의 상태는 good 으로 설정된다.
15. 사서는 Bad 책을 폐기한다.
16. 대여 신청 기능은 타MSA 죽어도 항상 동작해야함
17. Bad인 경우 벌금을 입금해야 반납 접수가 된다.
18. 벌금 입금되면 반납자 등급을 good으로 수정한다.
29. 대여 중인 경우 반납이 되어야 대여 할 수 있다.


비기능적 요구사항
1. 트랜잭션
    1. 대여 요청 시 책 보유량이 부족할 경우 대여가 되지 않는다. (Sync 호출) (ㅇ)
    2. 결제가 되지 않은 주문건은 아예 거래가 성립되지 않아야 한다 (Sync 호출) 
1. 장애격리
    1. 고객/마케팅/배달 관리 기능이 수행되지 않더라도 대여신청은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    2. 고객시스템이 과중되면 사용자를 잠시동안 받지 않고 재접속하도록 유도한다  Circuit breaker, fallback
1. 성능
    1. 관리센터는 관리페이지를 통해 대여상태를 확인할 수 있어야 한다  CQRS (ㅇ)
